...
2. 유저가 입력한 세가지 숫자를 받는다.
    * 123 -> 각 자리수를 어떻게 세가지 인트로 받을까..?
        ...
        * 923 / 10 = 92, 92 % 10 = 2 -> 2가 답이었다 수식 오류
...
### 개발 후 노트
1. 막연하게 a ≠ b 면 b ≠ c 도 될거라고 생각했는데 a ≠ c 도 체크 했어야 헀다. 
    * 첫째자리와 셋째 자리가 같은 숫자들 랜덤으로 생성되는 케이스 발견
    * c1 과 c3 이 중복되는지도 체크 추가
2. 위 생각했던 수식에서 오류가 있어 십의 자리수가 제대로 구해지지 않은 버그발생했었다.
    * (u2 / 10) / 10 -> (u1 / 10) % 10 으로 수정
3. 맥에서는 이모지를 넣고 돌려도 콘솔창 출력이 문제 없었는데, 윈도에서는 콘솔창에서 이모지가 깨져보인다.
